...
function CopyBtn({ text, label, style }) {
  ...
  return (
    ...
    }}>{done ? `✓ ${t.nav.copied}` : label}</button>
  )
}
...
function InviteQr({ text }) {
  ...
  return (
    <img src={dataUrl} alt={t.school.qrAlt} width={180} height={180}
...
export default function School() {
  ...
  if (user?.role !== 'owner') return (
    <div style={{ maxWidth: 480, margin: '60px auto', textAlign: 'center' }}>
      <div style={{ fontSize: 44 }}>🔒</div><h2>{t.school.teacherOnly}</h2>
      <p style={{ color: 'var(--ink-soft)' }}>{t.school.teacherOnlyHint}</p>
    </div>
  )
  ...
  const createClass = async () => {
    setBusy(true); setErr('')
    try {
      const c = await api.post('/classes', { name: newName.trim() || t.school.newClassDefault })
      setNewName(''); setCreating(false); await load(); openClass(c.id)
    } catch (e) { setErr(e.message) } finally { setBusy(false) }
  }
  const removeStudent = async (uid) => {
    if (!window.confirm(t.school.removeConfirm)) return
  ...
  return (
    <div style={{ maxWidth: 940, margin: '0 auto', padding: '18px 16px 60px' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-0.5px' }}>{t.school.title}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1.2fr)', gap: 20, alignItems: 'start' }} className="school-grid">
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            }}>{creating ? '✕' : t.school.addClassBtn}</button>
          </div>
          ...
          {creating && (
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              ...
              <button onClick={createClass} disabled={busy} style={{
                ...
                background: 'var(--accent)', color: 'var(--accent-ink)', fontWeight: 700, fontSize: 13,
              }}>{busy ? '…' : t.school.createBtn}</button>
            </div>
          )}

          {!classes && <div style={{ color: 'var(--ink-soft)' }}>{t.common.loading}</div>}
                ...
                <div key={c.id} onClick={() => openClass(c.id)} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--ink-soft)' }}>
                      {t.school.studentsCount(c.students)} · {t.school.codeLabel} <b style={{ color: 'var(--accent)', letterSpacing: '1px' }}>{c.invite_code}</b>
                    </div>
                  </div>
        ...
        <div>
          ...
          {selected === 'loading' && <div style={{ color: 'var(--ink-soft)' }}>{t.common.loading}</div>}
          {selected && selected !== 'loading' && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 16, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '18px 20px', background: 'linear-gradient(135deg, rgba(201,165,74,0.12), rgba(124,92,255,0.10))' }}>
                <div style={{ fontSize: 12, color: 'var(--ink-soft)', fontWeight: 600, marginBottom: 6 }}>{t.school.inviteCode}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <CopyBtn text={selected.invite_code} label={t.school.codeBtn} />
                  <CopyBtn text={joinLink(selected.invite_code)} label={t.school.linkBtn} />
                </div>
              ...
              <div style={{ padding: '14px 20px 18px' }}>
                ...
                {(!selected.members || selected.members.length === 0) && (
                  <div style={{ color: 'var(--ink-soft)', fontSize: 14, padding: '10px 0' }}>{t.school.nobodyYet}</div>
                )}
                    ...
                    return (
                      <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 10px', borderRadius: 10, background: 'var(--surface-2)' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--ink-soft)' }}>{isTeacher ? t.nav.teacher : t.nav.student}</div>
                        </div>
                        {!isTeacher && (
                          <button onClick={() => removeStudent(m.id)} title={t.school.removeTitle}
