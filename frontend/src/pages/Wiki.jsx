    ...
    teacher: [
      {
        icon: '1️⃣',
        title: 'Регистрация',
        body: 'Зарегистрируйтесь с ролью «Учитель / Родитель». Это даёт доступ к созданию уроков, просмотру учеников и управлению курсами.',
      },
      {
        icon: '2️⃣',
        title: 'Создайте курс',
        body: 'Перейдите в «Курсы» → «+ Курс». Введите название (например "6 класс 2024/2025"). Курс — это папка для уроков.',
      },
      {
        icon: '3️⃣',
        title: 'Загрузите урок',
        body: 'Внутри курса нажмите «+ Добавить урок». Или через «+ Урок» в меню. Сфотографируйте страницы учебника и/или тетради (можно несколько фото). Необязательно: добавьте аудиозапись урока. Нажмите «Обработать урок».',
      },
      {
        icon: '⏳',
        title: 'Ожидание обработки',
        body: 'Claude AI читает фото и создаёт карточки. На каждое фото ~10 секунд. 24 фото ≈ 5-8 минут. Прогресс виден на странице: «Фото 3 из 24...» → «Создаю упражнения...» → «Готово!». Страницу можно не держать открытой — обработка идёт на сервере. Статус видно в разделе «Уроки».',
      },
      {
        icon: '4️⃣',
        title: 'Что создаётся',
        body: 'Из каждого слова в уроке Claude создаёт 4 упражнения:\n🃏 Флеш-карта — перевернуть и оценить\n✏️ Заполни пропуск — вписать слово в предложение\n☑️ Выбери ответ — 4 варианта\n✍️ Напиши предложение — своё предложение, Claude проверит',
      },
      {
        icon: '5️⃣',
        title: 'Ученики',
        body: 'Ученики регистрируются сами с ролью «Ученик». Их прогресс виден в разделе «Ученики»: сколько слов выучено, сколько попыток сегодня.',
      },
      {
        icon: '🔑',
        title: 'API ключи',
        body: 'Ключи хранятся в файле .env на сервере. ANTHROPIC_API_KEY обязателен для обработки фото. OPENAI_API_KEY нужен только для расшифровки аудио.',
      },
    ],
    student: [
      {
        icon: '1️⃣',
        title: 'Регистрация',
        body: 'Зарегистрируйтесь с ролью «Ученик». Спросите у учителя адрес сайта.',
      },
      {
        icon: '2️⃣',
        title: 'Раздел "Сегодня"',
        body: 'Здесь показаны упражнения для повторения на сегодня. Видно сколько каждого типа ждёт. Нажмите «Начать повторение» чтобы начать сессию.',
      },
      {
        icon: '🃏',
        title: 'Флеш-карта',
        body: 'Показывается немецкое слово. Вспомни перевод, потом нажми на карточку чтобы увидеть ответ. Оцени насколько хорошо помнишь:\n❌ Не помню — повторится сегодня\n😐 С трудом — повторится через 1 день\n✅ Помню — повторится через 6+ дней',
      },
      {
        icon: '✏️',
        title: 'Заполни пропуск',
        body: 'В предложении есть пропуск ___. Введи правильное слово и нажми «Проверить». Регистр не важен.',
      },
      {
        icon: '☑️',
        title: 'Выбери ответ',
        body: 'Показывается вопрос и 4 варианта ответа. Нажми на правильный. Зелёный = верно, красный = неверно. Через секунду автоматически переходит к следующему.',
      },
      {
        icon: '✍️',
        title: 'Напиши предложение',
        body: 'Дано слово и подсказка. Составь любое простое предложение на немецком с этим словом. Claude проверит: правильно ли использовано слово, нет ли грубых ошибок. Получишь оценку (★☆☆☆☆) и комментарий.',
      },
      {
        icon: '📊',
        title: 'Интервальное повторение',
        body: 'Программа сама решает когда повторять слова. Чем лучше помнишь — тем реже показывает. Слова делятся на:\n🆕 Новое — ещё не изучалось\n📖 Изучается — в процессе\n✅ Выучено — помнишь хорошо (5+ повторений подряд)',
      },
    ],
  },
  de: {
...
export default function Wiki() {
  ...
  const [tab, setTab] = useState(0)  // 0 = teacher, 1 = student
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 28 }}>
        {content.tabs.map((label, i) => (
          <button key={i} onClick={() => setTab(i)} style={{
            ...
            backgroundColor: tab === i ? '#4f46e5' : '#e5e7eb',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {sections.map((sec, i) => (
          <div key={i} style={{ border: '1px solid #e5e7eb', borderRadius: 10, padding: '16px 20px', backgroundColor: '#fff' }}>
            <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              ...
              <div>
                ...
                <div style={{ color: '#374151', fontSize: 14, lineHeight: 1.6, whiteSpace: 'pre-line' }}>{sec.body}</div>
              </div>
            </div>
          </div>
        ))}
