...
export default function LessonReport() {
  ...
  if (err) return <div style={{ maxWidth: 720, margin: '40px auto', color: 'var(--red, #d64545)' }}>{tt.common.error}: {err}</div>
  if (!data) return <div style={{ maxWidth: 720, margin: '40px auto', color: 'var(--ink-soft)' }}>{tt.common.loading}</div>

  const { lesson, totals, group, students, notStarted } = data

  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '16px 12px 60px' }}>
      <button onClick={() => navigate(-1)} style={{ background: 'none', border: 'none', color: 'var(--accent)', cursor: 'pointer', fontSize: 14, marginBottom: 8 }}>← {tt.nav.back}</button>
      <h2 style={{ margin: '0 0 4px' }}>{tt.reports.lessonReportTitle(lesson.title)}</h2>
      ...
      <div style={card}>
        <h3 style={{ margin: '0 0 10px', fontSize: 16 }}>{tt.reports.groupStuck}</h3>
        {group.hardWords.length === 0 ? (
          <div style={{ color: 'var(--ink-soft)', fontSize: 13 }}>{tt.reports.noMassErrors}</div>
        ) : (
      ...
      {/* Блок Б — по ученикам */}
      <div style={card}>
        <h3 style={{ margin: '0 0 10px', fontSize: 16 }}>{tt.reports.byStudents}</h3>
        {students.length === 0 ? (
          <div style={{ color: 'var(--ink-soft)', fontSize: 13 }}>{tt.reports.nobodyPassed}</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      ...
      {notStarted.length > 0 && (
        <div style={card}>
          <h3 style={{ margin: '0 0 10px', fontSize: 16 }}>{tt.reports.notStarted(notStarted.length)}</h3>
