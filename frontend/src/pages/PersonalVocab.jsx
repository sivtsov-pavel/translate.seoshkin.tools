...
const STATUS = {
  new:      { key: 'statusNew',      color: 'var(--ink-soft)' },
  learning: { key: 'statusLearning', color: '#B07D1B' },
  known:    { key: 'statusKnown',    color: 'var(--good, #16a34a)' },
}
...
export default function PersonalVocab() {
  ...
  const upload = async (files) => {
    ...
    try {
      ...
      setMsg(r.added.length ? t.myWords.addedMsg(r.added.length) + (r.skipped ? t.myWords.alreadyWas(r.skipped) : '') : t.myWords.noNewWords + (r.skipped ? t.myWords.alreadyExist(r.skipped) : ''))
      load()
  ...
  const remove = async (id) => {
    if (!window.confirm(t.myWords.removeConfirm)) return
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '18px 16px 60px' }}>
      <div style={{ background: 'linear-gradient(135deg, rgba(59,122,87,0.16), rgba(124,92,255,0.12))', border: '1px solid var(--line)', borderRadius: 18, padding: '22px', marginBottom: 18 }}>
        <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-0.5px' }}>{t.myWords.title}</div>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 14 }}>
        <button onClick={() => camRef.current?.click()} disabled={uploading} style={btn('accent')}>
          {uploading ? t.myWords.parsing : t.myWords.photoBtn}
        </button>
        <button onClick={() => fileRef.current?.click()} disabled={uploading} style={btn('ghost')}>{t.myWords.fromFiles}</button>
      ...
      {/* Список */}
      {!words && <div style={{ color: 'var(--ink-soft)' }}>{t.common.loading}</div>}
          ...
          return (
            <div key={w.id} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: 12, display: 'flex', gap: 12, alignItems: 'center' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                }}>{t.myWords[st.key]}</button>
              </div>
              <button onClick={() => remove(w.id)} title={t.myWords.removeTitle} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink-soft)', fontSize: 15 }}>✕</button>
            </div>
          )
        })}
      </div>
