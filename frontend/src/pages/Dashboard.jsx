      ...
      {needsSchedule.length === 1 ? (
        <div onClick={() => navigate(`/courses/${needsSchedule[0].id}`)} style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--accent)', fontWeight: 600 }}>{t.dashboard.needsScheduleDesc(needsSchedule[0].title)}</div>
          </div>
        </div>
      ) : (
        <div style={{ background: 'var(--accent-soft)', border: '2px solid var(--accent)', borderRadius: 16, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 28 }}>📅</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 800, fontSize: 15, color: 'var(--ink)' }}>{t.dashboard.needsScheduleMultiTitle}</div>
            <div style={{ fontSize: 13, color: 'var(--accent)', fontWeight: 600, marginBottom: 8 }}>{t.dashboard.needsScheduleMultiDesc}</div>
  ...
  return (
    <div style={{ paddingBottom: 90 }}>
      ...
      {(() => {
        ...
        // «Только сегодня»: показываем ОДИН ТЕКУЩИЙ урок — первый НЕ пройденный по порядку
        // (фронтир дрипа: прошёл 1,2 → текущий 3, а не последний по номеру). Пройденные берём из
        // /exercises/completed-lessons. Если всё пройдено — показываем последний.
        const completedIds = new Set((completed || []).map(c => c.id))
        const byNumAsc = [...booksAll].sort((a, b) => (a.lesson_number ?? 1e9) - (b.lesson_number ?? 1e9) || a.lesson_id - b.lesson_id)
        const current = byNumAsc.find(l => !completedIds.has(l.lesson_id)) || byNumAsc[byNumAsc.length - 1] || null
