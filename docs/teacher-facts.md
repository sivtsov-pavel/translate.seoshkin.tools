...
> ⚠️ Раздел не претендует на исчерпывающий построничный аудит всех файлов в
> `frontend/src/pages/` — ряд страниц без явных grep-совпадений на роль
> (`Report.jsx`, `Phrasebook.jsx`, `Grammar.jsx`, `WordMatch.jsx`, `Crossword.jsx`,
> `Profile.jsx`, `LoveKids.jsx`, `Tutors.jsx`, `TextReader.jsx`) просмотрены
> бегло, а не построчно. Список ниже — то, что подтверждено кодом, не гарантия
> полноты по всем разделам приложения.
