...
const COURSE = { id: "de", label: "Немецкий", flag: "🇩🇪", stripe: ["#1a1a1a", "#d00", "#ffce00"] };
    ...
    <div className="dl-root">
      ...
      <div className="dl-topbar">
        ...
        <div className="dl-course-label">
          <span>{COURSE.flag}</span> {COURSE.label}
          <span className="dl-stripe-mini" style={{ background: `linear-gradient(90deg, ${COURSE.stripe.join(",")})` }} />
        </div>
        <button className="dl-switch-course">Сменить курс</button>
      </div>
        ...
        <div className="dl-drawer-overlay" onClick={() => setMenuOpen(false)}>
          <div className="dl-drawer" onClick={(e) => e.stopPropagation()}>
            <div className="dl-drawer-head">
              <span>{COURSE.flag} {COURSE.label}</span>
      ...
      {/* ---------- ТУР ---------- */}
      {tourStep >= 0 && <TourOverlay step={tourSteps[tourStep]} stepIndex={tourStep} total={tourSteps.length} onNext={nextTourStep} onSkip={() => setTourStep(-1)} />}
    </div>
  );
}
...
.dl-root{ position:relative; min-height:600px; background:var(--cream); max-width:520px; margin:0 auto; padding-bottom:60px; }
