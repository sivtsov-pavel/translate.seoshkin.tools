...
⚠️ **Только что завершена соседняя задача «свой ключ OpenAI у учителя»** (коммит на проде). Она добавила в страницу настроек секцию «🔑 Интеграции» (`frontend/src/pages/Settings.jsx` + `frontend/src/store/settings.js`) и эндпоинты `settings.js` (`/api/settings/openai-key*`), поля шифруются (`services/secretBox.js`, `openaiClient.js`). **Не трогай эту секцию/файлы шифрования** — свои тумблеры уведомлений добавляй отдельным блоком настроек, не ломая существующие. `GET/PATCH /api/settings` уже НЕ содержат `openai_key` (это намеренно).
