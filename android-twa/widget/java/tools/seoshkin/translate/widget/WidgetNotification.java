 ...
 *
 * 1b. setSilent(true) — ловушка. Он не просто убирает звук: система помечает уведомление
 *    как «тихое» и уносит в отдельный раздел, который Pixel на экране блокировки не
 *    показывает совсем. Именно поэтому карточка оказывалась «в сгруппированных», а
 *    обычный push от того же приложения был виден. Тишину задаём каналом — setSound(null)
 *    и выключенная вибрация — а само уведомление остаётся обычным.
 *
