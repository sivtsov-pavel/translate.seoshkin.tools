...
export async function exercisesRoutes(fastify) {
  ...
  fastify.post('/api/exercises/:id/attempt', {
    ...
    // Вся цепочка (SM-2, статус слова, попытка, снятие из хвостов) живёт в services/attempts.js:
    // тем же путём идут ответы с виджета домашнего экрана, второй копии быть не должно.
    return await recordAttempt(request.user.id, exerciseId, userAnswer, quality)
  })
