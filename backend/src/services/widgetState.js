...
export async function buildWidgetState(userId, schoolId, role, targetLang = 'de') {
  ...
  return {
    ...
    today,
    // Тап по виджету ведёт СРАЗУ В ЗАНЯТИЕ, а не на обзор урока: человек уже отвечал на
    // карточки прямо на домашнем экране, и попадать после этого в список типов —
    // лишний шаг в никуда (жалоба Павла 09.09.2026: «перехожу в набор упражнений, а не
    // в конкретное упражнение»). Сессия продолжает с неотвеченного, так что попадает он
    // ровно туда, где остановился.
    //
    // Путь БЕЗ query-строки намеренно: нативная часть склеивает его с «?from=widget»
    // (см. docs/WIDGET_BRIEF_FOR_GEMINI.md), и второй знак вопроса сломал бы разбор.
    // /start/:id — тонкий редирект во фронте, он же держит совместимость со старыми APK.
    nextUrl: lesson && base.state === WIDGET_STATES.IN_PROGRESS ? `/start/${lesson.id}` : '/',
