...
export function checkExercise(ex) {
  ...
  // В вопросе «выбери ответ» СЛОВО (после двоеточия) обязано быть на изучаемом языке.
  // Кириллица там означает, что генератор подставил русский перевод вместо слова — вопрос
  // показывает ответ сам («Wie heißt das auf Russisch: разговор?»), упражнение ничему не учит.
  // Рамка вопроса при этом может быть законно русской («Как перевести на русский: auch?») —
  // поэтому проверяем только хвост после двоеточия, а не весь вопрос.
  if (ex.type === 'multiple_choice' && ex.target_lang !== 'ru') {
    const q = String(p.question || '')
    const tail = q.includes(':') ? q.split(':').pop() : ''
    if (/[Ѐ-ӿ]/.test(tail)) bad('warn', `русское слово в вопросе: «${q.slice(0, 60)}»`)
  }

  // «Выбери ответ» без привязки к слову: карточка выходит без картинки и перевода.
  // У диктанта/фраз word_id может отсутствовать законно, у multiple_choice — нет.
  if (ex.type === 'multiple_choice' && ex.word_id == null) {
    bad('warn', 'нет привязки к слову — карточка без картинки и перевода')
  }
