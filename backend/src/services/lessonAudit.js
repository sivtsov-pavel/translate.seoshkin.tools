...
export function checkExercise(ex) {
  ...
  // В вопросе «выбери ответ» слово обязано быть на изучаемом языке. Кириллица в вопросе
  // означает, что генератор подставил русский перевод вместо слова — вопрос показывает
  // ответ сам («Wie heißt das auf Russisch: разговор?»), упражнение ничему не учит.
  // Реальный случай: урок 12 de, слово «разговор» вместо «das Gespräch».
  if (ex.type === 'multiple_choice' && ex.target_lang !== 'ru' && /[Ѐ-ӿ]/.test(String(p.question || ''))) {
    bad('warn', `русское слово в вопросе: «${String(p.question).slice(0, 60)}»`)
  }
